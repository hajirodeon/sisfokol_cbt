...
if ($_POST['btnSMP'])
	...
	if ((empty($e_user)) OR (empty($e_nama)))
		...
		if ($s == "baru")
			{
			//insert
			mysql_query("INSERT INTO adminx(kd, usernamex, passwordx, nama, postdate) VALUES ".
		...
		if ($s == "edit")
			{
			mysql_query("UPDATE adminx SET usernamex = '$e_user', ".
							"passwordx = '$e_pass', ".
							"nama = '$e_nama', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysql_query("DELETE FROM adminx ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysql_query("SELECT * FROM adminx ".
						"WHERE kd = '$kd'");
